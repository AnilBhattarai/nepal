...
roleController.getTotalProperty = async (req, res, next) => {
  try {
    const agency = await userSch.findById(req.user.id);
    const agency_id = (agency && agency.agent && agency.agent.agency) || false;
    let property_count = 0;
    let un_verified_project_count = 0;
    let in_active_project_count = 0;
    let project_count = 0;
    if (agency_id) {
      property_count = await propertySch.count({ is_deleted: false, is_project: false, agency_id: agency_id });
      project_count = await propertySch.count({ is_deleted: false, is_project: true, agency_id: agency_id });
      in_active_project_count = await propertySch.count({ is_deleted: false, is_project: false, is_active: false, agency_id: agency_id });
      un_verified_project_count = await propertySch.count({ is_deleted: false, is_project: false, is_verified: false, agency_id: agency_id });
    }
...
roleController.getActiveVsSoldProperty = async (req, res, next) => {
  try {
    const agency = await userSch.findById(req.user.id);
    const agency_id = (agency && agency.agent && agency.agent.agency) || false;
    const not_sold_out_count = await propertySch.count({ is_deleted: false, is_sold_out: false, agency_id: agency_id });
    const sold_out_count = await propertySch.count({ is_deleted: false, is_sold_out: true, agency_id: agency_id });
...
roleController.getPendingProperty = async (req, res, next) => {
  try {
    const agency = await userSch.findById(req.user.id);
    const agency_id = (agency && agency.agent && agency.agent.agency) || false;
    const pending_count = await propertySch.count({ is_deleted: false, is_active: true, is_verified: false, is_project: false, agency_id: agency_id });
