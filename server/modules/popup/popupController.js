...
popupController.getPopup = async (req, res, next) => {
  try {
    ...
    if (req.query.is_advertisement) {
      searchQuery = { ...searchQuery, is_advertisement: true };
    } else {
      searchQuery = { ...searchQuery, is_advertisement: { $ne: true } };
    }
...
popupController.getPopupDropdown = async (req, res, next) => {
  try {
    let { page, size, populate, selectQuery, searchQuery, sortQuery } = otherHelper.parseFilters(req, 10, false);
    searchQuery = { ...searchQuery, is_advertisement: true, is_active: true };
    let data = await popupSch.find(searchQuery);
    return otherHelper.sendResponse(res, httpStatus.OK, true, data, null, popupConfig.get, null);
  } catch (err) {
    next(err);
  }
};
...
popupController.selectMultipleData = async (req, res, next) => {
  const { popup_id, type } = req.body;

  if (type == 'is_active') {
    const Data = await popupSch.updateMany({ _id: { $in: popup_id } }, [
      {
        $set: {
    ...
    const Data = await popupSch.updateMany(
      { _id: { $in: popup_id } },
      {
        $set: {
          is_deleted: true,
...
popupController.getPopupDetail = async (req, res, next) => {
  try {
    ...
    let populateq = [
      ...
      { path: 'templateRequirement.image', select: 'filename size path mimetype ' },
    ];
