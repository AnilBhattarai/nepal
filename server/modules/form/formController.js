...
formController.getForm = async (req, res, next) => {
  try {
    ...
    if (req.query.find_full_name) {
      searchQuery = {
        full_name: { $regex: req.query.find_full_name, $options: 'i' },
        ...searchQuery,
      };
    }
    ...
    const datas = await otherHelper.getquerySendResponse(formSch, page, size, sortQuery, searchQuery, selectQuery, next, populate);
