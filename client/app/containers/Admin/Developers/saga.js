...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/developers'));
}
...
function* deleteDeveloper(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `user/developers/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Delete delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* developersSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteDeveloper);
