...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/road-type'));
}
...
function* deleteRoadType(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `enum/road_type/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Road type delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* roadTypeSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteRoadType);
