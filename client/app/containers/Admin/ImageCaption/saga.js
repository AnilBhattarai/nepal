...
function* loadAll(action) {
  ...
  yield call(
    Api.get(
      `enum/image_caption?${query}`, // "?" le error fhalna sakcha hai
...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/image_caption/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/image-caption'));
}
...
function* deleteImageCaption(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `enum/image_caption/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'image_caption delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* imageCaptionSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteImageCaption);
