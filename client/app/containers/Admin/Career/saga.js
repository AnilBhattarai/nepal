...
function* loadAppliedUsers(action) {
  const token = yield select(makeSelectToken());
  let query = '';
  if (action.payload) {
    Object.keys(action.payload).map(each => {
  ...
  yield call(
    Api.get(
      `careers/admin/appliedusers`,
      actions.loadAppliedUsersSuccess,
      actions.loadAppliedUsersFailure,
      token,
    ),
  );
}
...
function* addEdit() {
  ...
  yield fork(
    Api.post(
      'careers/admin/publisher',
      actions.addEditSuccess,
      actions.addEditFailure,
      { ...data, slug_url: data.job_title.replace(/[' ']/g, '-') },
      token,
    ),
  );
...
export default function* careerSaga() {
  ...
  yield takeLatest(types.LOAD_APPLIED_USERS_REQUEST, loadAppliedUsers);
