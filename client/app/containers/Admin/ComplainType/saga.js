...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/complain_type/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/complain_type_manage'));
}
...
function* deleteComplainType(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `enum/complain_type/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Complain type delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* complainTypeSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteComplainType);
