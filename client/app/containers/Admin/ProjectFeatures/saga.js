...
function* loadAll(action) {
  ...
  yield call(
    Api.get(
      `enum/project_features?${query}`, // "?" le error fhalna sakcha hai
...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/project_features/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/project-features'));
}
...
function* deleteProjectFeatures(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `enum/project_features/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Project Feature delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* projectFeaturesSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteProjectFeatures);
