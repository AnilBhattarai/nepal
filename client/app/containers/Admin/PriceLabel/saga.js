...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/price_label/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/price-label'));
}
...
function* deletePriceLabel(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `enum/price_label/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Price Label delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* priceLabelSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deletePriceLabel);
