...
export const RequestManagement = props => {
  const {
  ...
  const handleAdd = () => {
    clearOne();
    push('/admin/request-manage/add');
  };

  const handleEdit = id => {
    push(`/admin/request-manage/edit/${id}`);
  };
  ...
  const tableData = data.map(({ _id, name, email, message, purpose }) => [
    name,
    email,
    message,
    purpose.title,
  ...
  return (
    <>
      ...
      <div className="flex justify-between mt-3 mb-3">
        <PageHeader>Requests Management </PageHeader>
        <Fab
          color="primary"
          aria-label="Add"
          ...
          {data.length < 1 ? (
            <>
              ...
              <div className="border-red-500 border text-red-600 font-bold bg-red-100 px-4 py-2 mt-10 rounded">
                There are no wanted Properties yet.
              </div>
            </>
          ) : (
              <>
                ...
                <Table
                  tableHead={['Name', 'Email', 'Message', 'Purpose', '']}
                  tableData={tableData}
