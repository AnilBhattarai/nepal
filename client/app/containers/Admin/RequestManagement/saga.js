...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/request-manage'));
}
...
function* deleteMyRequest(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `myrequest/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'MyRequest delete success',
    options: {
      variant: 'success',
    },
  };
...
function* loadPurpose() {
  ...
  yield call(
    Api.get(
      ...
      actions.loadPurposeSuccess,
      actions.loadPurposeFailure,
      token,
    ),
  );
}
...
function* loadPrice() {
  ...
  yield call(
    Api.get(
      'enum/price_label',
      actions.loadPriceSuccess,
      actions.loadPriceFailure,
      token,
    ),
  );
}
...
export default function* requestManagementSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteMyRequest);
  ...
  yield takeLatest(types.LOAD_PRICE_REQUEST, loadPrice);
}
