...
export const initialState = {
  ...
  query: { size: 10, find_name: '', state_id: '' },
  loading: false,
  active_status: {
    loading: false,
    id: '',
  ...
  produce(state, draft => {
    switch (action.type) {
      ...
      case types.LOAD_STATE_SUCCESS:
        ...
        break;
      case types.ADD_VDCS:
        draft.one.vdcs = [...draft.one.vdcs, { name: '' }];
        break;
      case types.SET_VDCS:
        draft.one.vdcs[action.payload.index].name = action.payload.value;
        break;
      case types.DELETE_VDCS:
        draft.one.vdcs.splice(action.payload.index, 1);
        break;
