...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/property_face/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/property-face'));
}
...
function* deletePropertyFace(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `enum/property_face/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Property face delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* propertyFaceSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deletePropertyFace);
