...
function* loadAll(action) {
  ...
  yield call(
    Api.get(
      `enum/project_status?${query}`, // "?" le error fhalna sakcha hai
...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/project_status/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/project-status'));
}
...
function* deleteProjectStatus(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `enum/project_status/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Project Status delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* projectStatusSaga() {
  ...
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteProjectStatus);
