...
function* loadAll(action) {
  ...
  yield call(
    Api.get(
      `enum/length_unit?${query}`, // "?" le error fhalna sakcha hai
...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/length_unit/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/LengthUnit'));
}
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Length-Unit delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* LengthUnitSaga() {
