...
const key = 'lengthUnit';

export const LengthUnit = props => {
  const {
  ...
  const handleAdd = () => {
    clearOne();
    push('/admin/LengthUnit/add');
  };

  const handleEdit = id => {
    push(`/admin/LengthUnit/edit/${id}`);
  };
  ...
  const tableData = data.map(
    ({ _id, title, value, order, description, is_active }) => [
      title,
      value,
      order,
      description,
  ...
  return (
    <>
      <Helmet>
        <title>Length Unit Manage</title>
      </Helmet>
      <DeleteDialog
        open={open}
        doClose={handleClose}
      ...
      <div className="flex justify-between mt-3 mb-3">
        ...
        <PageHeader> LengthUnit </PageHeader>
        <Fab
          color="primary"
          aria-label="Add"
