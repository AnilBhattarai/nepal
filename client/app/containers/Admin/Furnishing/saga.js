...
function* loadAll(action) {
  ...
  yield call(
    Api.get(
      `enum/furnishing?${query}`, // "?" le error fhalna sakcha hai
...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/furnishing/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/furnishing'));
}
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'furnishing delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* furnishingSaga() {
