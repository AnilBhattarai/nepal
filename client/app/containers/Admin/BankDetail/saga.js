...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `bankDetailForLoan/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Bank details delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* bankDetailSaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteBank);
