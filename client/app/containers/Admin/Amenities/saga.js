...
function* loadAll(action) {
  ...
  yield call(
    Api.get(
      `enum/amenities?${query}`, // "?" le error fhalna sakcha hai
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/amenities'));
}
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Amenities delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* amenitiesSaga() {
  ...
  yield takeLatest(types.DELETE_ONE_REQUEST, deleteAmenities);
