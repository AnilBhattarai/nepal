    ...
    return (
      <div className="w-full flex flex-wrap">
        {properties.map((property, index) => (
          <SortableItem
            key={
              one.is_project ? property.id.project_id : property.id.property_id
            }
            index={index}
            value={
              <>
                <div>
                  <img
                    ...
                    alt={one.is_project ? 'Project' : 'Property'}
                  />
                </div>

                <CardContent>
                  <Typography component="p" style={{ minHeight: '40px' }}>
                    {one.is_project ? 'Project id:' : 'Property id:'}{' '}
                    {one.is_project
                      ? property.id.project_id
                      : property.id.property_id}{' '}
                    <br />
                    Title:
                    {(property.id.basic && property.id.basic.title) || ''}
                  </Typography>
                  {/* <FormControlLabel
                        control={
                          <Checkbox
...
const AddEdit = props => {
  ...
  const insertProperties = event => {
    ...
    one.properties.map(each => {
      ...
      // pid == tempProperties ? (found = true) : null;
      if (pid === parseInt(tempProperties, 10)) {
        found = true;
      } else {
        return null;
      }
    ...
    if (one.properties.indexOf(tempProperties) === -1 && found === false) {
      // setOneValue({
      //   key: 'properties',
      //   value: [...one.properties, tempProperties],
      // });
      if (one.is_project) {
        loadProjectRequest(tempProperties);
      } else {
        loadPropertyRequest(tempProperties);
      }
      setPropertiesValue('');
    }
    return { tempProperties: setPropertiesValue('') };
  };
      ...
      <div>
        <div className="flex justify-between mt-3 mb-3">
          <PageHeader>
            ...
            {match && match.params && match.params.id
              ? `Edit ${one.is_project ? 'Project' : 'Property'} Section`
              : `Add ${one.is_project ? 'Project' : 'Property'} Section`}
          </PageHeader>
        </div>
        <PageContent>
          <div className="w-full md:w-1/2 pb-4">
            ...
            >
              {one.is_project ? 'Project' : 'Property'} Section
            </label>
          ...
          <div className="w-full md:w-1/2 pb-4">
            ...
            >
              {one.is_project ? 'Projects' : 'Properties'}
          ...
          {window.location.pathname.includes('add') && (
            <FormControlLabel
              control={
                <Checkbox
                  checked={one.is_project || false}
                  tabIndex={-1}
                  onClick={handleCheckedChange('is_project')}
                  color="primary"
                />
              }
              label="Is Project"
...
AddEdit.propTypes = {
  ...
  tempProperties: PropTypes.string.isRequired,
};
...
const mapStateToProps = createStructuredSelector({
  ...
  tempProperties: makeSelectTempProperties(),
  searchResults: makeSelectSearchResult(),
  propertyLoading: makeSelectPropertyLoading(),
});
