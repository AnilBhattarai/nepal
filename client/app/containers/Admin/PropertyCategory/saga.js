...
function* loadOne(action) {
  ...
  yield call(
    Api.get(
      `enum/property_category/${action.payload}`,
...
function* redirectOnSuccess() {
  ...
  yield put(push('/admin/property-category'));
}
...
function* addEdit() {
  ...
  let main_data = {};
  if (data.wanted_image && data.wanted_image._id !== undefined) {
    main_data = { ...data, wanted_image: data.wanted_image._id };
  } else {
    main_data = { ...data };
  }
  yield fork(
...
function* deletePropertyCategory(action) {
  const token = yield select(makeSelectToken());
  yield call(
    Api.delete(
      `enum/property_category/${action.payload}`,
...
function* deleteSuccessFunc(action) {
  const snackbarData = {
    message: action.payload.msg || 'Property category delete success',
    options: {
      variant: 'success',
    },
  };
...
export default function* propertyCategorySaga() {
  yield takeLatest(types.LOAD_ALL_REQUEST, loadAll);
  yield takeLatest(types.LOAD_ONE_REQUEST, loadOne);
  yield takeLatest(types.ADD_EDIT_REQUEST, addEdit);
  yield takeLatest(types.DELETE_ONE_REQUEST, deletePropertyCategory);
