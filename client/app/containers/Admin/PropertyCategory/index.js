...
export const PropertyCategory = props => {
  ...
  const handleAdd = () => {
    clearOne();
    push('/admin/property-category/add');
  };

  const handleEdit = id => {
    clearOne();
    push(`/admin/property-category/edit/${id}`);
  };
  ...
  const tableData = data.map(
    ({ order, is_active, title, media, description, _id }) => [
      media ? (
        <img
  ...
  return (
    <>
      <Helmet>
        <title>Property Category Manage</title>
      </Helmet>
      <DeleteDialog
        open={open}
        doClose={handleClose}
      ...
      <div className="flex justify-between mt-3 mb-3">
        ...
        <PageHeader>Property Category Manage</PageHeader>
        <Fab
          color="primary"
          aria-label="Add"
