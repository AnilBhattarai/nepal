...
export function* redirectOnSuccess(redirect) {
  ...
  if (redirect) {
    ...
    // do something
    yield put(push('/user/profile/verify'));
  }
}
...
export function* signupAction(action) {
  ...
  const mobile_no = yield select(makeSelectMobile());
  const data = { email, password, name, mobile_no };
  ...
  if (errors.isValid) {
    ...
    yield put(actions.setStoreValue({ key: null, value: false }));
  }
}
