...
export const MyRequest = props => {
  ...
  const handleAdd = () => {
    clearOne();
    push('/user/request/add');
  };

  const handleEdit = id => {
    push(`/user/request/edit/${id}`);
  };
  ...
  const tableData = data
    .reverse()
    .map(({ _id, name, email, message, purpose }) => [
      name,
      email,
      message,
      purpose.title,
  ...
  return (
    <>
      ...
      <div className="flex justify-between mt-3 mb-3">
        <PageHeader>Wanted Properties </PageHeader>
        <Fab
          color="primary"
          aria-label="Add"
        ...
        <div className={`${loading ? 'opacity-50' : ''}`}>
          {data.length < 1 ? (
            <div className="border-red-500 border text-red-600 font-bold bg-red-100 px-4 py-2 mt-10 rounded">
              You have not posted any wanted Properties yet.
            </div>
          ) : (
              <>
