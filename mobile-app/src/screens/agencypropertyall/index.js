...
import { selectPropertyByAgencyAll, selectDataLoading, selectDataLoadMore } from '../../redux/property/property.selectors';
import { createStructuredSelector } from 'reselect';
import { connect } from 'react-redux';
import { propertyByAgencyDataAll, setLoadMore } from '../../redux/property/property.actions';
import tempImg3 from '../../../assets/home.png';
import { IMAGE_URL } from '../../api';

export class AllAgencyProperty extends Component {
  state = {
    refreshing: false,
    isInfiniteRefreshing: false,
  };
  async componentDidMount() {
    await this.props.propertyByAgencyDataAll(this.props.navigation.getParam('id'));
  }
  onRefresh = async () => {
    await this.props.propertyByAgencyDataAll(this.props.navigation.getParam('id'));
    ...
    if (this.state.isInfiniteRefreshing) {
      ...
      if (page + 1 <= totalPages) {
        Promise.resolve(this.props.setLoadMore(true)).then(
          this.props.propertyByAgencyDataAll(`${this.props.navigation.getParam('id')}&page=${page + 1}`),
        );
  ...
  render() {
    const { data, loadmore, loading } = this.props;
...
const mapStateToProps = createStructuredSelector({
  data: selectPropertyByAgencyAll,
  loading: selectDataLoading,
  loadmore: selectDataLoadMore,
});
const mapDispatchToProps = {
  propertyByAgencyDataAll,
  setLoadMore,
};

export default connect(mapStateToProps, mapDispatchToProps)(AllAgencyProperty);
